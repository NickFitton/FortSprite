...
Postgres runs locally in Docker and persists its data in the `postgres_data` Docker volume.

```bash
# Copy the example configuration once, then add your Clerk keys.
...
The default `DATABASE_URL` in `.env.example` connects to the local container.

```bash
pnpm db:status # Check the container and health status
pnpm db:logs   # Follow Postgres logs
pnpm db:down   # Stop the container, preserving data
pnpm db:reset  # Delete local database data and start fresh
```
